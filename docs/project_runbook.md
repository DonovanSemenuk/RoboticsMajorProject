...
# Run the selected Nav2 tour after discovery.
ros2 launch tour_guide tour.launch.py

# Hardware fallback if Nav2 touring is not reliable on the robot.
ros2 launch tour_guide hardware_tour.launch.py

# Optional operator override for the hardware fallback.
ros2 run teleop_twist_keyboard teleop_twist_keyboard --ros-args -r /cmd_vel:=/cmd_vel_key
```

For the physical robot, replace the simulation bringup with the OU TurtleBot 4 bringup and the cardboard-city map. Prefer the same `discover.launch.py` and `tour.launch.py` workflow when Nav2 is working; use `hardware_tour.launch.py` when you need the direct controller adapted from the working Project 2 hardware code.
...
- Make sure every outside code source is credited, especially `ros2_aruco` and the Project 2 hardware controller patterns.
