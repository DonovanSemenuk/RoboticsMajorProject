...
\end{enumerate}
The same high-level workflow applies on the physical robot, except that the real TurtleBot~4 bringup and map for the cardboard-city environment replace the Gazebo simulation. If Nav2 goal execution is unreliable during the hardware demonstration, the project also includes a direct hardware tour driver adapted from the team's working Project~2 controller patterns: stamped velocity commands on \texttt{/cmd\_vel}, odometry yaw feedback, \texttt{/scan} safety halts, and \texttt{/cmd\_vel\_key} keyboard override priority \cite{project2_controller}.
...
\begin{itemize}
    ...
    \item \texttt{tour\_guide/hardware\_tour\_node.py}: hardware fallback that drives through the selected route using odometry, stamped velocity commands, lidar safety stops, and keyboard override priority.
